    ...
    <div className="min-h-screen bg-[#f7f9fb]">
      <header className="border-b border-[#dde3ea] bg-white shadow-sm">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-6 text-sm font-medium text-[#374151] sm:flex">
            ...
            {isFreelancer ? (
              <Link href="/projects/explore" className="hover:text-[#007bd2]">
                Explorar proyectos
              </Link>
            ) : (
              <Link href="/projects/new" className="hover:text-[#007bd2]">
                Publicar proyecto
              </Link>
            )}
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mb-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <StatCard
            label={isFreelancer ? "Proyectos abiertos" : "Mis proyectos"}
          ...
          {isFreelancer ? (
            ...
            <>
              ...
              <ActionCard
                ...
                description="Encuentra profesionales por especialidad, país y tarifa."
                href="#"
                disabled
              />
            </>
        ...
        {profile && (
          <div className="rounded-lg border border-[#dde3ea] bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              ...
              {profile.hourlyRate && (
                <ProfileField label="Tarifa" value={`$${profile.hourlyRate}/hora`} />
              )}
              {profile.bio && (
...
function ActionCard({ title, description, href, disabled }: { title: string; description: string; href: string; disabled: boolean }) {
  ...
  const content = (
    <>
      <div className="flex items-center justify-between">
        ...
        {disabled ? (
          <span className="rounded-full bg-[#eef2f6] px-2.5 py-0.5 text-xs font-medium text-[#6b7280]">Pronto</span>
        ) : (
