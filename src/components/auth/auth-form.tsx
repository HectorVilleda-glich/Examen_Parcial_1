...
export function AuthInput({ id, label, type = "text", name, required = true, minLength, placeholder }: {
  id: string; label: string; type?: string; name: string; required?: boolean; minLength?: number; placeholder?: string;
}) {
  return (
    <div>
      <label htmlFor={id} className="mb-1.5 block text-sm font-medium text-[#374151]">{label}</label>
      <input id={id} name={name} type={type} required={required} minLength={minLength} placeholder={placeholder}
...
export function AuthSubmitButton({ children, loading, loadingText }: { children: React.ReactNode; loading: boolean; loadingText: string }) {
  return (
